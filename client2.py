...
class ClientJSON:
    def __init__(self, host='localhost', port=8088):
        ...
        self.file_upload_url = f"http://{host}:{port}/api/upload" # url for file uploads
    ...
    def send_file_upload_request(self, file_path):
        # uploads a file and retrieves file URL
        print(f"Attempting to upload file: {file_path}") # debugging
        ...
        try:
            with open(file_path, 'rb') as f:
                files = {'file': f}
                response = requests.post('http://localhost:8088/api/upload', files=files, timeout = 10)

                print(f"Upload response status code: {response.status_code}") #debugging
                print(f"Upload response text: {response.text}")

                print(f"Response received: {response.status_code}") #debugging
                if response.status_code == 200:
                    # successful file upload
                    file_url = response.json().get("file_url")
                    print(f"File successfully uploaded. Dowload URL: {file_url}")
                    return file_url
                elif response.status_code == 413:
                    print(f"Upload failed. File is too large.")
                else:
    ...
    def retrieve_file_request(self, file_url, save_path):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if option == "1":
            ...
            file_url = client.send_file_upload_request(file_path)
            if file_url:
            ...
            client.retrieve_file_request(file_url, save_path)
        elif option == "5":
