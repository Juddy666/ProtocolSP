...
class ServerJSON:
    def __init__(self, host='localhost', port=8088, server_port=8090, http_port=8000, neighbour_addresses=None):
        ...
        self.address = (host, port)              # For client connections
        self.server_address = (host, server_port)  # For server-to-server connections
        self.http_address = (host, http_port)      # For HTTP server (file transfers)
        ...
        self.server_timeout = 30      # Server timeout in seconds

        # Create sockets for client and server connections
        self.server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.server_server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

        # Bind sockets and start listening
        try:
            ...
            # Do not start accept_client_connections thread if binding fails
        ...
        try:
            ...
            # Do not start accept_server_connections thread if binding fails
    ...
    def handle_server_connection(self, server_socket, server_address):
        """Handle communication with a connected server."""
        data_buffer = ""
        self.servers[server_address] = {'socket': server_socket, 'last_heartbeat': time.time()}
        while True:
            try:
                ...
                if not data:
                    ...
                    break
                data_buffer += data
                # Parse JSON messages from the buffer
                while True:
                    try:
    ...
    def send_server_hello(self, server_socket):
        """Send server_hello message to a connected server."""
        message = {
            "type": "server_hello",
            "data": {
                "type": "server_hello",
    ...
    def process_server_message(self, server_socket, server_address, message):
        """Process messages received from other servers."""
    ...
    def handle_server_hello(self, server_socket, server_address, message):
        """Handle server_hello message from another server."""
    ...
    def send_client_update_request(self, server_socket):
        """Request client list from a connected server."""
        message = {
    ...
    def handle_client_update(self, server_socket, server_address, message):
        """Handle client_update message from another server."""
    ...
    def handle_client_update_request(self, server_socket):
        """Send client_update message with current clients to a server that requested it."""
    ...
    def handle_server_heartbeat(self, server_address):
        """Update last heartbeat time upon receiving a heartbeat from a server."""
        if server_address in self.servers:
            self.servers[server_address]['last_heartbeat'] = time.time()

    def server_heartbeat_check(self):
        """Periodically send heartbeat messages to servers and check for timeouts."""
        while True:
    ...
    def remove_server(self, server_address):
        """Remove a disconnected server and its associated clients."""
        if server_address in self.servers:
    ...
    def process_message(self, sender_socket, message, from_server=False):
        """Process messages received from clients or servers."""
        message_type = message.get('type')
    ...
    def handle_hello_message(self, client_socket, message_data):
        """Handle hello message from a new client."""
    ...
    def broadcast_client_update(self):
        """Broadcast client updates to connected servers."""
        client_update_message = {
            "type": "client_update",
            "clients": [client_info['public_key'] for client_info in self.clients.values()]
        }
    ...
    def broadcast_client_update_to_clients(self):
        """Broadcast client updates to connected clients."""
        client_update = {
            "type": "client_update",
            "clients": [client_info['public_key'] for client_info in self.clients.values()]
        }
        for client_info in self.clients.values():
            ...
            try:
                client_socket.sendall(json.dumps(client_update).encode('utf-8'))
            except Exception as e:
                print(f"Error sending client update to client: {e}")
                self.remove_client(client_socket)
    ...
    def send_client_list(self, client_socket):
        """Send the client list to a requesting client."""
        client_list = {
            "type": "client_list",
            "servers": []
        }
    ...
    def handle_public_chat(self, sender_socket, message, from_server=False):
        ...
        if not self.verify_signature(message, from_server):
    ...
    def handle_private_chat(self, sender_socket, message, from_server=False):
        ...
        for idx, recipient_fingerprint in enumerate(participants[1:]):  # Exclude sender
            if recipient_fingerprint in self.clients:
                ...
                if recipient_info:
                    ...
                    host, port = server_address_str.split(':')
    ...
    def verify_signature(self, message, from_server=False):
    ...
    def start_http_server(self):
        """Start the HTTP server for file transfers."""
